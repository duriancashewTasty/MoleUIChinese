...
struct OptimizeView: View {
    ...
    private var headerCard: some View {
        MoleHeroPanel(
            eyebrow: LanguageManager.tr("Health"),
            title: LanguageManager.tr("Optimize"),
             subtitle: LanguageManager.tr("Run health checks first, then apply the fixes you actually want to keep around. The page should feel more like a cockpit than a debug pane."),
            symbol: "bolt.circle.fill"
        ) {
            VStack(alignment: .trailing, spacing: 10) {
                if let score = computedHealthScore {
                    MoleMetricBadge(
                        title: LanguageManager.tr("Score"),
                        value: "\(score)",
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                HStack(spacing: 24) {
                    VStack(alignment: .leading, spacing: 4) {
                        Text("\(summary.completedCount)")
                            .font(.title2.bold())
                            .foregroundStyle(.green)
                        Text(LanguageManager.tr("Tasks Completed"))
                            .font(.caption)
                            .foregroundStyle(.secondary)
                    }
                    ...
                    if summary.skippedCount > 0 {
                        VStack(alignment: .leading, spacing: 4) {
                            ...
                            Text(LanguageManager.tr("Tasks Skipped"))
                                .font(.caption)
                                .foregroundStyle(.secondary)
                        }
                    }
                ...
                if !summary.skippedTasks.isEmpty {
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        Label(LanguageManager.tr("Skipped Tasks"), systemImage: "exclamationmark.triangle")
                ...
                if !summary.systemStatus.isEmpty {
                    Divider()
                    VStack(alignment: .leading, spacing: 6) {
                        Label(LanguageManager.tr("System Status"), systemImage: "info.circle")
                            .font(.subheadline.bold())
                ...
                if !summary.warnings.isEmpty {
                    Divider()
                    VStack(alignment: .leading, spacing: 6) {
                        Label(LanguageManager.tr("Suggestions"), systemImage: "lightbulb")
                            .font(.subheadline.bold())
                            .foregroundStyle(.orange)
                ...
                if !dryRunMode {
                    ...
                    Button {
                        ...
                        Label(LanguageManager.tr("View Detailed Log"), systemImage: "doc.text")
                            .font(.caption)
                    }
                    .buttonStyle(.plain)
                }
    ...
    private func rawOutputSheet(_ output: String) -> some View {
        VStack(spacing: 0) {
            HStack {
                Text(LanguageManager.tr("Detailed Execution Log"))
                    .font(.headline)
                Spacer()
                Button(LanguageManager.tr("Close")) {
                    showRawOutput = false
                }
            }
            .padding()
        ...
        for line in lines {
            ...
            if trimmed.contains("◎") {
                ...
                if trimmed.contains("Close these apps") {
                    summary.skippedTasks.append(LanguageManager.tr("Database Optimization: Close Safari first"))
                } else if trimmed.contains("Skipped font cache") {
                    summary.skippedTasks.append(LanguageManager.tr("Font Cache Rebuild: Close browsers first"))
    ...
    private func systemInfoHeader(_ report: HealthReport) -> some View {
        GroupBox {
            VStack(alignment: .leading, spacing: 6) {
                HStack {
                    Label(LanguageManager.tr("System Health"), systemImage: "heart.text.square")
                        .font(.headline)
                }
                HStack(spacing: 24) {
                    VStack(alignment: .leading, spacing: 2) {
                        Text(LanguageManager.tr("Memory")).font(.headline)
                        Text(String(
                            format: "%.1f / %.1f GB",
                            report.memoryUsedGb,
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        Text(LanguageManager.tr("Disk")).font(.headline)
                        Text(String(
                            format: "%.1f / %.1f GB (%.0f%%)",
                            report.diskUsedGb,
                            report.diskTotalGb,
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        Text(LanguageManager.tr("Uptime")).font(.headline)
    ...
    private func taskList(_ tasks: [OptimizationTask]) -> some View {
        ...
        return GroupBox {
            VStack(alignment: .leading, spacing: 0) {
                Label(LanguageManager.tr("Optimizations"), systemImage: "bolt.fill")
                    .font(.headline)
                    .padding(.bottom, 8)
                        ...
                        label: {
                            HStack(spacing: 8) {
                                Image(systemName: "exclamationmark.triangle")
                                    .foregroundStyle(.orange)
                                Text(LanguageManager.tr("Advanced Tasks"))
                                    .fontWeight(.medium)
