...
struct DiskAnalyzerView: View {
    ...
    var body: some View {
        VStack(spacing: 16) {
            MoleHeroPanel(
                eyebrow: LanguageManager.tr("Storage"),
                title: LanguageManager.tr("Disk Analyzer"),
                subtitle: LanguageManager.tr("Inspect the current folder visually, then drill in only where the size story looks suspicious."),
                symbol: "internaldrive"
            ) {
                VStack(alignment: .trailing, spacing: 10) {
                    MoleMetricBadge(
                        title: LanguageManager.tr("Current Path"),
                        value: scanner.currentPath.lastPathComponent,
                        systemImage: "folder.fill",
                        tint: MoleTheme.sky
                    )
                    MoleMetricBadge(
                        title: LanguageManager.tr("Visible Size"),
    ...
    private var contentArea: some View {
        if needsFullDiskAccessPrompt {
            ...
            ContentUnavailableView(
                "Empty Directory",
                systemImage: "folder",
                description: Text(LanguageManager.tr("No items found"))
            )
        } else {
            entryList
        }
    ...
    private var fullDiskAccessPrompt: some View {
        VStack(spacing: 16) {
            ...
            Text(LanguageManager.tr("Full Disk Access Recommended"))
            ...
            HStack(spacing: 10) {
                ...
                Button(LanguageManager.tr("Check Again")) {
                    retryPendingScan()
                }

                Button(LanguageManager.tr("Scan Anyway")) {
                    continuePendingScan()
                }
            }
        }
    ...
    private var entryList: some View {
        ...
        .alert(
            ...
            Button(LanguageManager.tr("Move to Trash"), role: .destructive) {
    ...
    private var fullDiskAccessPromptDetail: String {
        ...
        case .granted:
            LanguageManager.tr("Mole UI now appears to have Full Disk Access. You can continue the Home-folder scan.")
        case .notGranted:
            LanguageManager.tr("Scanning your Home folder without Full Disk Access causes macOS to interrupt the scan with separate folder prompts. Enable it first for a cleaner disk scan.")
        case .unknown:
            LanguageManager.tr("Mole UI could not positively verify Full Disk Access yet. If you just enabled it, click Check Again once before continuing.")
        }
    }
