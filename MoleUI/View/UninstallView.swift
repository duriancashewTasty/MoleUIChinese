...
enum AppSortOrder: String, CaseIterable {
    ...
    /// Localized display name for the sort picker
    var displayName: String {
        switch self {
        case .name: LanguageManager.tr("Name")
        case .size: LanguageManager.tr("Size")
        case .lastUsed: LanguageManager.tr("Last Used")
        }
    }
}
...
struct UninstallView: View {
    ...
    private var headerCard: some View {
        MoleHeroPanel(
            eyebrow: LanguageManager.tr("Applications"),
            title: LanguageManager.tr("Uninstall"),
             subtitle: LanguageManager.tr("Review installed apps with their size and last-use signal before you move anything to Trash."),
            symbol: "xmark.app.fill"
        ) {
            VStack(alignment: .trailing, spacing: 10) {
                HStack(spacing: 10) {
                    MoleSearchField(prompt: LanguageManager.tr("Search apps"), text: $searchText)
                        .frame(width: 200)

                    Picker(LanguageManager.tr("Sort"), selection: $sortOrder) {
                ...
                HStack(spacing: 10) {
                    if let lastScan = scanService.lastScanTime {
                        MoleMetricBadge(
                            title: LanguageManager.tr("Scanned"),
    ...
    private func appRow(_ app: AppInfo) -> some View {
        ...
        return GroupBox {
            HStack(spacing: 10) {
                ...
                if wasUninstalled {
                    Text(LanguageManager.tr("Removed"))
                        .font(.caption)
                        .foregroundStyle(.green)
                        .frame(width: 80)
                } else {
                    Button(LanguageManager.tr("Uninstall")) {
