...
struct DashboardView: View {
        ...
        Group {
            if let snap = service.snapshot {
                ...
                ContentUnavailableView(
                    LanguageManager.tr("Connection Error"),
                    systemImage: "exclamationmark.triangle",
                    description: Text(error)
                )
            } else {
                MoleLoadingState(title: LanguageManager.tr("Loading system metrics..."))
            }
        }
        .task {
    ...
    private func cpuCard(_ cpu: CPUStatus, thermal: ThermalStatus) -> some View {
        GroupBox {
            VStack(alignment: .leading, spacing: 6) {
                Label(LanguageManager.tr("CPU"), systemImage: "cpu").font(.headline)
                HStack {
                    Text(LanguageManager.tr("Total")).frame(width: 50, alignment: .leading)
                    UsageBar(cpu.usage)
                ...
                ForEach(Array(topCores), id: \.offset) { idx, val in
                    HStack {
                        Text(LanguageManager.tr("Core ") + "\(idx + 1)").frame(width: 50, alignment: .leading)
                        UsageBar(val)
                        Text(String(format: "%.1f%%", val)).monospacedDigit()
                    }
                }
                HStack {
                    Text(LanguageManager.tr("Load"))
    ...
    private func memoryCard(_ mem: MemoryStatus) -> some View {
        GroupBox {
            VStack(alignment: .leading, spacing: 6) {
                Label(LanguageManager.tr("Memory"), systemImage: "memorychip").font(.headline)
                HStack {
                    Text(LanguageManager.tr("Used")).frame(width: 50, alignment: .leading)
                    UsageBar(mem.usedPercent, color: .yellow)
                    Text(String(format: "%.1f%%", mem.usedPercent)).monospacedDigit()
                }
                HStack {
                    Text(LanguageManager.tr("Free")).frame(width: 50, alignment: .leading)
                    UsageBar(100 - mem.usedPercent, color: .green)
                    Text(String(format: "%.1f%%", 100 - mem.usedPercent)).monospacedDigit()
                }
                HStack {
                    Text(LanguageManager.tr("Total"))
                    Text("\(MetricsFormatter.humanBytes(mem.used)) / \(MetricsFormatter.humanBytes(mem.total))")
                }
                HStack {
                    Text(LanguageManager.tr("Cached") + " \(MetricsFormatter.humanBytes(mem.cached))")
                    Spacer()
                    Text(LanguageManager.tr("Avail") + " \(MetricsFormatter.humanBytes(mem.total - mem.used))")
                }
                .foregroundStyle(.secondary)
                Spacer(minLength: 0)
            }
    ...
    private func diskCard(_ disks: [DiskStatus], io: DiskIOStatus) -> some View {
        GroupBox {
            VStack(alignment: .leading, spacing: 6) {
                Label(LanguageManager.tr("Disk"), systemImage: "internaldrive").font(.headline)
                ...
                HStack {
                    Text(LanguageManager.tr("Read"))
                    Text(MetricsFormatter.formatRate(io.readRate))
                    Spacer()
                    Text(LanguageManager.tr("Write"))
    ...
    private func powerCard(_ batteries: [BatteryStatus], thermal: ThermalStatus) -> some View {
        GroupBox {
            VStack(alignment: .leading, spacing: 6) {
                Label(LanguageManager.tr("Power"), systemImage: "battery.75percent").font(.headline)
                if let bat = batteries.first {
                    HStack {
                        Text(LanguageManager.tr("Level")).frame(width: 50, alignment: .leading)
                    ...
                    Text(LanguageManager.tr("No battery")).foregroundStyle(.secondary)
                }
                Spacer(minLength: 0)
            }
            .font(.system(.caption, design: .monospaced))
    ...
    private func processCard(_ procs: [MoleProcessInfo]) -> some View {
        ...
        return GroupBox {
            VStack(alignment: .leading, spacing: 6) {
                Label(LanguageManager.tr("Processes"), systemImage: "list.bullet").font(.headline)
    ...
    private func networkCard(
        ...
        GroupBox {
            VStack(alignment: .leading, spacing: 6) {
                Label(LanguageManager.tr("Network"), systemImage: "network").font(.headline)
                HStack {
                    Text(LanguageManager.tr("Down")).frame(width: 40, alignment: .leading)
                ...
                HStack {
                    Text(LanguageManager.tr("Up")).frame(width: 40, alignment: .leading)
