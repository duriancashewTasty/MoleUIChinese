...
enum FullDiskAccessStatus: Equatable {
    ...
    var title: String {
        switch self {
        case .granted:
            LanguageManager.tr("Full Disk Access appears enabled")
        case .notGranted:
            LanguageManager.tr("Full Disk Access is not enabled")
        case .unknown:
            LanguageManager.tr("Full Disk Access could not be confirmed")
        }
    }
    ...
    var detail: String {
        switch self {
        case .granted:
            LanguageManager.tr("Broad disk scans should be able to inspect protected Library content without repeated folder-by-folder interruptions.")
        case .notGranted:
            LanguageManager.tr("macOS does not provide a one-shot prompt for Full Disk Access. Mole UI can guide you to the correct System Settings page so you can enable it yourself.")
        case .unknown:
            LanguageManager.tr("Mole UI could not positively verify Full Disk Access yet. Refresh after changing System Settings, or try Disk Analyzer again.")
        }
    }
}
...
struct SettingsView: View {
    ...
    @AppStorage("AppPreferredLanguage") private var preferredLanguage: String = "zh-Hans"
    ...
    private var heroCard: some View {
        MoleHeroPanel(
            eyebrow: LanguageManager.tr("Control Room"),
            title: LanguageManager.tr("Settings"),
            subtitle: LanguageManager.tr("Version details, disk-scan privacy guidance, and a short list of upstream CLI tools that still live outside this GUI."),
            symbol: "gearshape.2.fill"
        )
    }

    private var languageCard: some View {
        settingsCard(title: LanguageManager.tr("Language"), symbol: "globe") {
            VStack(alignment: .leading, spacing: 10) {
                HStack {
                    Text(LanguageManager.tr("App Language"))
                    Spacer()
                    Picker(LanguageManager.tr("App Language"), selection: $preferredLanguage) {
                    ...
                    .onChange(of: preferredLanguage) {
                        LanguageManager.shared.currentLanguage = preferredLanguage
                    }
                }
            }
        }
    ...
    private var aboutCard: some View {
        settingsCard(title: LanguageManager.tr("About"), symbol: "info.circle") {
            VStack(alignment: .leading, spacing: 14) {
                MoleVersionView(bundledCLIVersion: readMoleCLIVersion())

                HStack(alignment: .top) {
                    Text(LanguageManager.tr("GitHub"))
                    Spacer()
    ...
    private var permissionsCard: some View {
        settingsCard(title: LanguageManager.tr("Disk Access & Privacy"), symbol: "hand.raised") {
            VStack(alignment: .leading, spacing: 12) {
                ...
                HStack(spacing: 10) {
                    Button {
                        ...
                        Label(LanguageManager.tr("Open Full Disk Access"), systemImage: "lock.open.display")
                    }
                    .buttonStyle(.borderedProminent)

                    Button(LanguageManager.tr("Refresh Status")) {
                        refreshFullDiskAccessStatus()
                    }

                    Button(LanguageManager.tr("Open Privacy & Security")) {
                        openPrivacyAndSecuritySettings()
                    }
                }

                permissionTip(
                    title: LanguageManager.tr("Reduce repeated prompts"),
                    detail: LanguageManager.tr("Desktop, Documents, Downloads, iCloud, and parts of Library all have separate privacy rules. Full Disk Access is the most reliable way to avoid repeated denials during broad scans.")
                )
                permissionTip(
                    title: LanguageManager.tr("When Full Disk Access helps"),
                    detail: LanguageManager.tr("If you want to analyze your whole Home folder, app data, iCloud files, or deeper Library content, granting Full Disk Access will make Disk Analyzer much more consistent.")
                )
            }
        }
    }
    ...
    private var cliOnlyCard: some View {
        settingsCard(title: LanguageManager.tr("Selected Upstream CLI Tools"), symbol: "terminal") {
            VStack(alignment: .leading, spacing: 10) {
                Text(LanguageManager.tr("These are a few upstream Mole commands that stay outside the GUI. This is a guide to the most relevant ones, not a full command reference."))
                    .font(.system(size: 12))
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)

                cliFeatureRow("mo touchid", LanguageManager.tr("Configure Touch ID for sudo in the upstream CLI. This changes macOS sudo PAM config and stays outside MoleUI."))
                cliFeatureRow("mo completion", LanguageManager.tr("Generate or install shell completion scripts for bash, zsh, and fish."))
                cliFeatureRow("mo update", LanguageManager.tr("Self-update the upstream Mole CLI to the latest stable release."))
                cliFeatureRow("mo update --nightly", LanguageManager.tr("Switch the upstream CLI to the latest unreleased main build."))
                cliFeatureRow("mo remove", LanguageManager.tr("Uninstall the upstream Mole CLI from the system."))
            }
        }
    }
