...
struct MoleVersionView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                Label(LanguageManager.tr("Version Details"), systemImage: "app.badge")
                ...
                if versionChecker.isChecking {
                    ...
                    Button {
                        ...
                        Label(LanguageManager.tr("Check for Updates"), systemImage: "arrow.clockwise")
                            .font(.caption)
                    }
                    .buttonStyle(.link)
                }
            ...
            HStack(spacing: 12) {
                versionChip(
                    title: LanguageManager.tr("Mole UI"),
                    value: versionChecker.currentVersion ?? "Unknown",
                    systemImage: "app.badge.fill",
                    tint: .green
                )
                versionChip(
                    title: LanguageManager.tr("Bundled CLI"),
            ...
            if let latestVersion = versionChecker.latestVersion {
                HStack {
                    Text(LanguageManager.tr("Latest release"))
                ...
                if versionChecker.hasUpdate {
                    HStack {
                        Text(LanguageManager.tr("Update available"))
                            .font(.caption)
                            .foregroundColor(.orange)

                        Spacer()

                        Link(LanguageManager.tr("View Release"), destination: URL(string: "https://github.com/imnotnoahhh/MoleUI/releases/latest")!)
                            .font(.caption)
                    }
                } else {
                    Text(LanguageManager.tr("This build is already on the latest published Mole UI release."))
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
            } else if let updateError = versionChecker.updateError {
                Text(LanguageManager.tr("Update check failed: ") + updateError)
                    .font(.caption)
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
                    .textSelection(.enabled)
            } else {
                Text(LanguageManager.tr("Latest release has not been checked yet."))
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
        }
