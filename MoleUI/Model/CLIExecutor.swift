...
final class CLIExecutor {
    ...
    enum ExecutionError: LocalizedError, Equatable {
        ...
        var errorDescription: String? {
            switch self {
            case .timeout:
                LanguageManager.tr("Operation Timeout")
            case .cancelled:
                LanguageManager.tr("Operation Cancelled")
            case .commandNotFound(let cmd):
                LanguageManager.tr("The '\(cmd)' command could not be found")
            case .nonZeroExit(let code, let stderr):
                LanguageManager.tr("Command failed (exit code: \(code))\n\(stderr)")
            case .invalidOutput(let msg):
                LanguageManager.tr("Output Parsing Failed: \(msg)")
            }
        }
...
extension CLIExecutor {
    ...
    func executeAndParseJSON<T: Decodable>(
        ...
        guard let data = result.stdout.data(using: .utf8) else {
            throw ExecutionError.invalidOutput(LanguageManager.tr("Cannot convert to UTF-8"))
        }
