    ...
    private func performScan(directory: URL) async throws -> ScanResult {
        guard let binary = findAnalyzeBinary() else {
            throw NSError(
                domain: "DiskModel",
                code: 1,
                userInfo: [NSLocalizedDescriptionKey: LanguageManager.tr("Cannot find analyze-go executable")]
            )
        }
            ...
            process.terminationHandler = { process in
                ...
                if process.terminationStatus != 0 {
                    ...
                    let error = NSError(
                        ...
                        userInfo: [
                            NSLocalizedDescriptionKey: message?.isEmpty == false ? message! : LanguageManager.tr("Disk scan failed"),
                        ]
                    )
                    continuation.resume(throwing: error)
                    return
