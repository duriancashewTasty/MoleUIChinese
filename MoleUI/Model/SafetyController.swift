...
final class SafetyController {
    ...
    func executeClean(
        ...
        if !dryRun {
            try await requestConfirmation(
                title: LanguageManager.tr("Confirm Clean"),
                message: LanguageManager.tr("About to clean \(target), total \(preview.formattedSize)"),
                destructive: true,
                preview: preview
            ) {
            ...
            lastResult = ExecutionResult(
                success: true,
                message: LanguageManager.tr("Preview mode: will clean \(preview.formattedSize)"),
    ...
    func executeOptimize(
        ...
        try await requestConfirmation(
                title: LanguageManager.tr("Confirm Optimize"),
                message: LanguageManager.tr("About to execute \(target) optimization"),
            destructive: false,
            preview: nil
        ) {
    ...
    private func performClean(target: String, dryRun: Bool) async throws {
        isExecuting = true
        progress = 0
        progressMessage = LanguageManager.tr("Cleaning...")
        ...
        lastResult = ExecutionResult(
            success: true,
            message: LanguageManager.tr("Clean completed"),
    ...
    private func performOptimize(target: String) async throws {
        isExecuting = true
        progress = 0
        progressMessage = LanguageManager.tr("Optimizing...")
        ...
        lastResult = ExecutionResult(
            success: true,
            message: LanguageManager.tr("Optimize completed"),
            details: result.stdout,
            cleanedSize: nil,
            duration: duration
        )
...
struct PreviewSection: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                Label(LanguageManager.tr("Preview"), systemImage: "eye")
                    .font(.subheadline)
                Spacer()
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    if preview.files.count > 10 {
                        Text(LanguageManager.tr("... and \(preview.files.count - 10) more files"))
                            .font(.caption)
                            .foregroundColor(.secondary)
                    }
                }
